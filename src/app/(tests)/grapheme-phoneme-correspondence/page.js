...
const GraphemeTestContent = () => {
  ...
  const onCompleteHandler = useCallback(
    (score) => {
      toast.info(
        `Test completed with score: ${score}. Suppressing results page as requested.`
      );
  ...
  const handleSubmitFinal = async () => {
    ...
    if (!childId) {
      ...
      console.warn("handleSubmitFinal: Aborted - childId or token missing.", {
        childIdExists: !!childId,
        tokenExists: !!token,
      });
    ...
    console.log(
      "handleSubmitFinal: Processing started. Current userInputs (from hook):",
      userInputs
    );
    ...
    console.log(
      "handleSubmitFinal: Prepared finalUserInputs for submission:",
      finalUserInputs
    );
    try {
      // Create the userResponses object that the backend expects
      const userResponses = {};
      ...
      const payload = {
        childId,
        userResponses, // This is what the backend expects
        language: langKey,
      };
      console.log(
        "handleSubmitFinal: Sending POST request to /api/grapheme-test/submitResult with payload:",
        payload
      );
      ...
      toast.dismiss();
      console.log(
        "handleSubmitFinal: API call successful. Status:",
        evalResponse.status,
        "Response data:",
        evalResponse.data
      );
      if (
        ...
        (typeof evalResponse.data.score === "number" ||
          typeof evalResponse.data.totalScore === "number")
      ) {
        const newScore =
          evalResponse.data.score ?? evalResponse.data.totalScore;
        setScore(newScore);
        console.log("handleSubmitFinal: Score set to:", newScore);

        if (suppressResultPage && typeof onCompleteHandler === "function") {
          console.log(
            "handleSubmitFinal: Suppressing result page and calling onCompleteHandler."
          );
        ...
        console.error(
          "handleSubmitFinal: Invalid response structure from API. Score missing.",
          evalResponse.data
        );
        setTestStage("submit");
      }
      ...
      if (error.response) {
        console.error(
          "handleSubmitFinal: API Error - Status:",
        ...
        console.error(
          "handleSubmitFinal: Network Error - No response received. Request details:",
          error.request
        );
      } else {
        console.error(
          "handleSubmitFinal: Request Setup Error - Message:",
          error.message
        );
      }
      setTestStage("submit");
      ...
      setIsProcessingFinalSubmit(false);
      console.log(
        "handleSubmitFinal: Processing finished. isProcessingFinalSubmit set to false."
      );
    }
  };
  ...
  const getBackButtonText = () => {
    switch (testStage) {
      ...
      case "practice":
      case "practiceCompleted":
        return t("backToIntro") || "Back to Intro";
      case "test":
      case "submit":
        return t("exitToMenu") || "Exit to Menu";
      case "results":
        return t("backToMenu") || "Back to Menu";
      default:
        return t("back") || "Back";
  ...
  if (testStage === "intro") {
    mainCardContent = (
      <div className="text-center py-12 min-h-[400px] flex items-center justify-center">
        ...
        <p className="ml-3 text-lg text-slate-300">Loading Introduction...</p>
      </div>
    );
  } else if (
    (testStage === "practice" || testStage === "practiceCompleted") &&
    ...
    mainCardContent = (
      <div className="text-center py-12 min-h-[400px] flex items-center justify-center">
        <p className="text-lg text-slate-300">
          Practice Complete! Modal will guide you.
        </p>
      </div>
    );
    ...
    mainCardContent = (
      <div className="text-center py-12 min-h-[400px] flex items-center justify-center">
        <Loader2 size={36} className="animate-spin text-sky-400" />
        <p className="ml-3 text-lg text-slate-300">Loading Test Letters...</p>
      </div>
    );
  } else if (
    testStage === "test" &&
    ...
    mainCardContent = (
      ...
      >
        ...
        <p className="ml-3 text-lg text-orange-300">Finalizing Test...</p>
      </div>
    );
  } else {
    mainCardContent = (
      <div className="text-center py-12 min-h-[400px] flex items-center justify-center">
        ...
        <p className="ml-3 text-lg text-gray-300">Loading...</p>
      </div>
    );
  }
  ...
  if (testStage === "intro") {
    return (
      <WelcomeDialog
        ...
        onStartTest={startPracticeAfterIntroDialog}
        t={t}
      />
    );
  }
          ...
          {testStage !== "intro" &&
            ...
            testStage !== "practiceCompleted" && (
              <>
                ...
                <div
                  className="absolute -bottom-10 -left-10 w-40 h-40 bg-purple-500/10 rounded-full filter blur-2xl animate-pulse"
