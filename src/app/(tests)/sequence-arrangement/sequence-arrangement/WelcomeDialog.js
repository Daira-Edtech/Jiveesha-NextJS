...
const WelcomeDialog = ({ t, speak, onEntireTestComplete, initialChildId }) => {
  ...
  useEffect(() => {
    if (internalGameState === "welcome" && dialogContent && dialogContent.length > 0) {
      speak(dialogContent[0]);
    }
  }, [internalGameState, speak]);
  ...
  const handleTryAgainRequest = useCallback(() => {
    if (internalGameState === 'practice') {
        setCurrentSequence([...practiceSequenceData]);
    } else if (internalGameState === 'test' && currentItemIndex < testItemsData.length) {
  ...
  const renderContent = () => {
    switch (internalGameState) {
      case "welcome":
        return (
          <>
            <div className="fixed inset-0 z-40"> {/* Overlay for welcome dialog focus */}
              <div
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 lg:p-8">
              ...
              >
                <motion.div /* Character */
                  ...
                  animate={{ y: 0, opacity: 1, scale: [1, 1.03, 1], rotate: [0, 2, -2, 0] }}
                  transition={{
                    y: { duration: 0.6, ease: "backOut" }, opacity: { duration: 0.8 },
                    scale: { duration: 4, repeat: Infinity, repeatType: "reverse", ease: "easeInOut" },
                    rotate: { duration: 8, repeat: Infinity, ease: "easeInOut" },
                  }}
                  className="flex-shrink-0 order-2 lg:order-1"
                >
                  <Image src={captainCharacter} alt="Captain" width={384} height={448}
                ...
                >
                  ...
                  <motion.div /* Dialog text */
                    key={`dialog-${currentDialogIndex}`} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
                  ...
                  <div className="flex justify-center gap-3 mb-8 lg:mb-10">
                    {dialogContent.map((_, index) => ( /* Progress dots */
                      <motion.div key={`dot-${index}`}
                        className={`w-3 h-3 sm:w-4 sm:h-4 rounded-full transition-all duration-300 ${index <= currentDialogIndex ? "bg-gradient-to-r from-white to-amber-200 shadow-lg" : "bg-white/30"}`}
                        initial={{ scale: 0.8 }} animate={{ scale: index === currentDialogIndex ? 1.3 : 1, y: index === currentDialogIndex ? -4 : 0 }}
                  ...
                  <div className="flex justify-center">
                    <motion.button /* Next button */
                      ...
                      className={`flex items-center justify-center gap-3 py-4 px-8 lg:px-12 rounded-xl font-bold text-lg lg:text-xl shadow-2xl transition-all duration-300 ${currentDialogIndex < dialogContent.length - 1 ? "bg-gradient-to-r from-white to-amber-100 text-amber-900 hover:from-amber-50 hover:to-amber-200" : "bg-gradient-to-r from-amber-500 via-orange-500 to-yellow-500 text-white hover:from-amber-600 hover:to-yellow-600"}`}
                    >
                      {currentDialogIndex < dialogContent.length - 1 ? <><span className="drop-shadow-sm">{t("continue")}</span><span className="text-xl">⏳</span></> : <><span className="drop-shadow-sm">{t("letsBegin")}</span><span className="text-xl">🦀</span></>}
                    </motion.button>
                  </div>
                </motion.div>
              </motion.div>
      ...
      case "preTestInstructions":
        return (
          <motion.div key={internalGameState} initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
            <InstructionsScreen
      ...
      case "test":
        return (
          <motion.div key={`${internalGameState}-gameplay`} initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="w-full h-full">
            <TopBar
            ...
            <div className="flex flex-col items-center justify-center min-h-screen relative px-2 sm:px-4 py-10 pt-20"> {/* pt-20 for TopBar */}
              <GameplayArea
                ...
                t={t}
                currentTestItemNumber={internalGameState === 'test' ? currentItemIndex + 1 : undefined}
                totalTestItems={internalGameState === 'test' ? testItemsData.length : undefined}
              />
            </div>
          </motion.div>
        );
      ...
      default:
        return <div className="text-white p-10">Error: Unknown game state: {internalGameState}</div>;
    }
  };
