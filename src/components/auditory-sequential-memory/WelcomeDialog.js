...
const WelcomeDialog = ({
  ...
  const parseTranscript = useCallback(
    (transcriptInput) => {
      ...
      const sortedWords = Object.keys(currentDigitMap).sort(
      ...
      if (language === "hi" || language === "kn") {
        ...
        const numberWords = Object.keys(currentDigitMap).filter(
          (word) => word.length > 1
        );
  ...
  const moveToNextSequence = useCallback(() => {
    console.log(
      "moveToNextSequence called - sequenceIndex:",
      sequenceIndex,
      "sequences.length:",
      sequences.length,
      "isPracticeMode:",
        ...
        if (isPracticeMode) {
          ...
          if (mode === "forward") {
            setForwardScore((prev) => {
              ...
              console.log(
                "Incrementing forward score from",
                prev,
                "to",
                newScore
              );
            ...
            setReverseScore((prev) => {
              const newScore = prev + 1;
              console.log(
                "Incrementing reverse score from",
                prev,
                "to",
                newScore
              );
        ...
        if (isPracticeMode) {
          ...
          if (mode === "forward") {
            setForwardErrors((prev) => {
              ...
              console.log(
                "Incrementing forward errors from",
                prev,
                "to",
                newErrorCount
              );
            ...
            setReverseErrors((prev) => {
              newErrorCount = prev + 1;
              console.log(
                "Incrementing reverse errors from",
                prev,
                "to",
                newErrorCount
              );
          ...
          setTimeout(() => {
            ...
            if (newErrorCount >= MAX_ERRORS) {
              if (mode === "forward") {
                console.log(
                  "Max errors reached in forward mode, moving to reverse"
                );
                setGameState("reverseInstructions");
              } else {
                console.log(
                  "Max errors reached in reverse mode, showing results"
                );
                setGameState("results");
              }
            } else {
  ...
  const handleFinishTest = useCallback(() => {
    ...
    const finalScore = {
      ...
      final: forwardScore + reverseScore,
    };
    onEntireTestFlowComplete(finalScore);
  }, [isSubmitting, forwardScore, reverseScore, onEntireTestFlowComplete]);
      ...
      default:
        return (
          <div className="text-white p-10">
            {t("error_unknown_game_state", { gameState })}
          </div>
        );
    }
  };
