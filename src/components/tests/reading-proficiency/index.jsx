  ...
  const submitTest = async (
    ...
    try {
      ...
      const responseFromApi = await axios.post(
        "/api/tests/reading-test/submitResult",
      ...
      if (responseFromApi.status === 201) {
        ...
        if (suppressResultPage && typeof onTestComplete === "function") {
          ...
          toast.success(t("testSubmittedWithScore").replace("{score}", score), {
            position: "top-center",
            onClose: () => {
              // PATH CHANGE: Navigating with Next.js router, passing state via query params
              router.push({
                pathname: "/results", // PATH CHANGE: Ensure this route exists in your Next.js app
...
export default function Test6Controller({ suppressResultPage = false, onComplete }) {
  ...
  useEffect(() => {
    ...
    if (t && tutorialMessages.length > 0 && tutorialMessages[0]) { // Ensure t and messages are loaded
        introSequence();
    }
    ...
    if (file) {
      ...
      if (currentTranscript) {
        ...
        const progress = Math.min(
          ((currentPage + 1) / Math.ceil(currentWords.length / wordsPerBatch)) * 85,
          85
        );
  ...
  const handleSubmitPageOrTest = async () => {
    if (!transcriptionReady && (!selectedFile && allTranscriptions[currentPage] === undefined) ) {
    ...
    if (allTranscriptions[currentPage]) { // If already transcribed (e.g. via file upload)
