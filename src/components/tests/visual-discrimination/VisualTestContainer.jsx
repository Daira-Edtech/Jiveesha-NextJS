...
const VisualTestContainer = ({ suppressResultPage = false, onComplete }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post(
        "/api/tests/visual-test/submitResult",
        {
          child_id: childId,
  ...
  if (quizQuestions.length === 0 && !quizStarted && !showCharacter && !quizCompleted) {
     return (
      <div
        className="fixed inset-0 overflow-y-auto flex items-center justify-center p-4 md:p-8 bg-cover bg-center"
        style={{ backgroundImage: `url(${backgroundImage})` }}
      >
        <p className="text-white text-2xl">{t("loadingTest") || "Loading test..."}</p>
      </div>
    );
  }
