...
import {
    FaInfoCircle, FaPlay, FaCheckCircle, FaTimesCircle, FaUndo, FaLightbulb,
    FaRegLightbulb, FaEye, FaMousePointer, FaHourglassHalf, FaSmileBeam, FaThumbsUp, FaRocket, FaGraduationCap,
    FaSpellCheck 
} from 'react-icons/fa';
...
const InstructionsModal = ({ isOpen, onClose, title, instructions, t }) => {
  ...
  const instructionIcons = [
    <span className="flex items-center"><FaEye className="mr-2 text-sky-300" /> <FaSpellCheck className="text-sky-300" /></span>, 
      ...
      >
        ...
        <div className="flex justify-between items-center mb-6 sm:mb-8 relative z-10">
          <h3 className="text-2xl sm:text-3xl font-bold text-amber-300 flex items-center drop-shadow-lg">
            <FaGraduationCap className="mr-3 text-amber-300 text-3xl sm:text-4xl" /> {title}
          </h3>
          <motion.button
          ...
          {instructions.map((item, index) => (
            ...
            >
              <div className="mr-3 sm:mr-4 text-2xl sm:text-3xl flex-shrink-0 mt-0.5">
                {instructionIcons[index] || <FaCheckCircle className="text-emerald-400" />}
              </div>
        ...
        <div className="flex justify-center relative z-10">
          <motion.button
            whileHover={{ scale: 1.05, y: -3, boxShadow: "0px 5px 20px rgba(251, 191, 36, 0.5)" }}
            whileTap={{ scale: 0.95 }}
            onClick={onClose}
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-white p-4">
        ...
        >🧩</motion.div>
        <p className="text-xl font-serif text-yellow-100">{t("loadingPractice")}</p>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full p-4 sm:p-6 w-full">
        <motion.div
          ...
          transition={{ type: 'spring', duration: 0.8, delay:0.1, bounce: 0.3 }}
        ...
        >
          ...
          <motion.button
            whileHover={{ scale: 1.08, y: -3, boxShadow: "0px 8px 25px rgba(251, 191, 36, 0.6)" }}
            whileTap={{ scale: 0.92 }}
  ...
  if (!demoQuestion) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-white text-xl p-6 bg-red-700/40 backdrop-blur-sm rounded-xl border border-red-500/60 shadow-lg">
         ...
         <span className="font-serif text-center">{t("errorNoPracticeQuestion")}</span>
      </div>
    );
  }
  ...
  return (
    <div className="relative w-full h-full flex flex-col items-center justify-center p-2 sm:p-4">
      <motion.button
        ...
        whileTap={{ scale: 0.95 }}
      >
        <FaInfoCircle size={20}/> <span className="hidden sm:inline">{t("demoInstructionsButton")}</span>
      </motion.button>
              ...
              <div className="relative z-10">
                <div className="flex flex-col items-center text-red-300 text-xl sm:text-2xl font-semibold mb-4">
                    <div className="flex items-center">
                        ...
                        <span className="drop-shadow-sm">{t("practiceIncorrect")}</span>
                    </div>
      ...
      <style jsx global>{`
        ...
        .custom-scrollbar-light::-webkit-scrollbar-track { background: rgba(255, 255, 255, 0.1); border-radius: 10px; }
        .custom-scrollbar-light::-webkit-scrollbar-thumb { background: rgba(251, 191, 36, 0.7); border-radius: 10px; }
        .custom-scrollbar-light::-webkit-scrollbar-thumb:hover { background: rgba(251, 191, 36, 0.9); }
      `}</style>
    </div>
  );
};
